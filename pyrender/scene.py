...
class Scene(object):
    ...
    def light_nodes(self):
        ...
        return (
            self.point_light_nodes
            | self.spot_light_nodes
        ...
        if self._bounds is None:
            ...
            if len(corners) == 0:
                ...
                self._bounds = np.array(
                    [np.min(corners, axis=0), np.max(corners, axis=0)]
                )
        return self._bounds
    ...
    def add(self, obj, name=None, pose=None, parent_node=None, parent_name=None):
        ...
        if parent_node is None and parent_name is not None:
            ...
            if len(parent_nodes) == 0:
                raise ValueError(
                    "No parent node with name {} found".format(parent_name)
                )
            elif len(parent_nodes) > 1:
                raise ValueError(
                    "More than one parent node with name {} found".format(parent_name)
                )
